document.querySelector('button[value="OkayFile"]').addEventListener("click", function (event) {
    const fileInput = document.querySelector('[name="lesson_content"]'); // انتخاب فیلد lesson_content
    ...
    if (fileInput.files.length !== 0) {
        ...
        if (fileInput.files.length > 0) {
            ...
            if (!allowedExtensions.includes(fileExtension)) {
                errorMessage.textContent = "فرمت فایل درس معتبر نیست. فقط فایل‌های mpg,mp3,mp4 مجازند.";
        ...
        errorMessage.textContent = " برای آپلود فایل انتخاب کنید!";
        errorMessage.style.display = "block";
        event.preventDefault();
        return;
    }
