...
def mycertificateView(request):
    try:

        progressed_courses = StudentProgress.objects.filter(student=request.user,completed_lessons_count__gt=0).select_related('course')
        courses_data = []
        for progress in progressed_courses:
            course = progress.course
            # if not course.certified:
            #     continue
    ...
    return render(request, "courses/user-page/mycertificate.html", {'courses_data': courses_data, 'breadcrumbs':breadcrumbs})
